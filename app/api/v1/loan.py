from fastapi import APIRouter, Depends, HTTPException
from sqlalchemy.orm import Session

from app.schemas.loan import LoanCreate, LoanResponse, LoanUpdate
from app.services.loan_service import LoanService
from app.api.dependecies import get_db, get_mongo
from app.db.mongo.mongo_client import MongoClientManager

router = APIRouter()


def get_loan_service(mongo: MongoClientManager = Depends(get_mongo)):
    return LoanService(mongo)


@router.post("/", response_model=LoanResponse)
def create_loan(
    data: LoanCreate,
    db: Session = Depends(get_db),
    service: LoanService = Depends(get_loan_service),
):
    return service.create(db, data)


@router.get("/", response_model=list[LoanResponse])
def get_loans(
    db: Session = Depends(get_db),
    service: LoanService = Depends(get_loan_service),
):
    return service.get_all(db)


@router.get("/{id_loan}", response_model=LoanResponse)
def get_loan(
    id_loan: int,
    db: Session = Depends(get_db),
    service: LoanService = Depends(get_loan_service),
):
    loan = service.get_by_id(db, id_loan)

    if not loan:
        raise HTTPException(status_code=404, detail="Loan not found")

    return loan


@router.put("/{id_loan}", response_model=LoanResponse)
def update_loan(
    id_loan: int,
    data: LoanUpdate,
    db: Session = Depends(get_db),
    service: LoanService = Depends(get_loan_service),
):
    loan = service.update(db, id_loan, data)

    if not loan:
        raise HTTPException(status_code=404, detail="Loan not found")

    return loan


@router.delete("/{id_loan}")
def delete_loan(
    id_loan: int,
    db: Session = Depends(get_db),
    service: LoanService = Depends(get_loan_service),
):
    success = service.delete(db, id_loan)

    if not success:
        raise HTTPException(status_code=404, detail="Loan not found")

    return {"message": "Loan deleted successfully"}
